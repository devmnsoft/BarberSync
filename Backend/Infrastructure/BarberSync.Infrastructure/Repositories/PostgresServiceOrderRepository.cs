...
public sealed class PostgresServiceOrderRepository(IDbConnectionFactory connections) : IServiceOrderRepository, IPaymentRepository
{
    private const string OpenOrderSql = """
        ...
        WHERE EXISTS (
            ...
            WHERE id=@client AND tenant_id=@tenant AND branch_id=@branch AND deleted_at IS NULL
        )
          AND (@appointment IS NULL OR EXISTS (
            SELECT 1 FROM barber.appointments
            WHERE id=@appointment AND tenant_id=@tenant AND branch_id=@branch
              AND client_id=@client AND deleted_at IS NULL
          ))
        """;
    ...
    private const string AddServiceItemSql = """
        ...
        WHERE id=@service AND tenant_id=@tenant AND (branch_id IS NULL OR branch_id=@branch)
          AND deleted_at IS NULL AND status='Active'
          AND EXISTS (
            SELECT 1 FROM barber.professionals p
            JOIN barber.professional_services ps ON ps.professional_id=p.id AND ps.service_id=@service
            WHERE p.id=@professional AND p.tenant_id=@tenant AND p.branch_id=@branch
    ...
    private const string AddProductItemSql = """
        ...
        WHERE id=@product AND tenant_id=@tenant AND branch_id=@branch
          AND deleted_at IS NULL AND status='Active'
    ...
    private const string UpdateItemSql = """
        ...
        WHERE i.id=@item AND i.service_order_id=@order AND i.tenant_id=@tenant AND i.branch_id=@branch AND i.deleted_at IS NULL
          AND @quantity>0 AND @discount>=0 AND @discount<=round(@quantity*i.unit_price,2)
          AND (i.product_id IS NULL OR EXISTS (
              SELECT 1 FROM barber.products p
              WHERE p.id=i.product_id AND p.tenant_id=@tenant AND p.branch_id=@branch
                AND (p.allow_negative_stock OR p.current_stock>=@quantity)
          ))
        """;
    ...
    { if(request.ClientId==Guid.Empty)throw new InvalidOperationException("Selecione um cliente válido.");await using var c=await connections.OpenConnectionAsync(ct);await using var tx=await c.BeginTransactionAsync(ct);var id=Guid.NewGuid();await using(var cmd=Command(c,OpenOrderSql,tx)){Add(cmd,"id",id);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);Add(cmd,"appointment",request.AppointmentId);Add(cmd,"client",request.ClientId);Add(cmd,"notes",request.Notes);if(await cmd.ExecuteNonQueryAsync(ct)!=1)throw new InvalidOperationException("Cliente ou agendamento inválido para esta unidade.");}await tx.CommitAsync(ct);return (await GetAsync(tenant,branch,id,ct))!; }
    ...
    { await using var c=await connections.OpenConnectionAsync(ct);await using var tx=await c.BeginTransactionAsync(ct);await EnsureOpen(c,tx,tenant,branch,id,ct);await using(var cmd=Command(c,UpdateItemSql,tx)){Add(cmd,"quantity",request.Quantity);Add(cmd,"discount",request.Discount);Add(cmd,"professional",request.ProfessionalId);Add(cmd,"item",itemId);Add(cmd,"order",id);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);if(await cmd.ExecuteNonQueryAsync(ct)!=1)throw new InvalidOperationException("Item não encontrado ou estoque insuficiente.");}await Recalculate(c,tx,tenant,branch,id,ct);await tx.CommitAsync(ct);return (await GetAsync(tenant,branch,id,ct))!; }

    public async Task<ServiceOrderResponse> RemoveItemAsync(Guid tenant,Guid branch,Guid user,Guid id,Guid itemId,RemoveOrderItemRequest request,CancellationToken ct)
    { if(string.IsNullOrWhiteSpace(request.Reason))throw new InvalidOperationException("Informe o motivo da remoção do item.");await using var c=await connections.OpenConnectionAsync(ct);await using var tx=await c.BeginTransactionAsync(ct);await EnsureOpen(c,tx,tenant,branch,id,ct);await using(var cmd=Command(c,"UPDATE barber.service_order_items SET deleted_at=now(),is_active=false,updated_at=now(),payload=payload||jsonb_build_object('removalReason',@reason,'removedBy',@user,'removedAt',now()) WHERE id=@item AND service_order_id=@order AND tenant_id=@tenant AND branch_id=@branch AND deleted_at IS NULL",tx)){Add(cmd,"item",itemId);Add(cmd,"order",id);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);Add(cmd,"reason",request.Reason.Trim());Add(cmd,"user",user);if(await cmd.ExecuteNonQueryAsync(ct)!=1)throw new KeyNotFoundException("Item não encontrado.");}await Recalculate(c,tx,tenant,branch,id,ct);await Audit(c,tx,tenant,branch,user,"ServiceOrder.ItemRemoved",id,$"Item {itemId} removido: {request.Reason.Trim()}",ct);await tx.CommitAsync(ct);return (await GetAsync(tenant,branch,id,ct))!; }
    ...
    {
        if (string.IsNullOrWhiteSpace(request.Code)) throw new InvalidOperationException("Informe o código do cupom.");
    ...
    {
        if (request.Amount <= 0) throw new InvalidOperationException("O valor de cashback deve ser maior que zero.");
        await using var c = await connections.OpenConnectionAsync(ct);
        await using var tx = await c.BeginTransactionAsync(System.Data.IsolationLevel.Serializable, ct);
        await EnsureOpen(c,tx,tenant,branch,id,ct);
        const string sql = """
    ...
    { var idempotencyKey=PaymentRules.ValidateIdempotencyKey(request.IdempotencyKey);await using var c=await connections.OpenConnectionAsync(ct);await using var tx=await c.BeginTransactionAsync(System.Data.IsolationLevel.Serializable,ct);
      await using(var existing=Command(c,"SELECT p.id,p.amount,p.status,o.status,o.total-COALESCE((SELECT sum(paid.amount) FROM barber.payments paid WHERE paid.service_order_id=o.id AND paid.status='Paid'),0) FROM barber.payments p JOIN barber.service_orders o ON o.id=p.service_order_id WHERE p.tenant_id=@tenant AND p.branch_id=@branch AND p.service_order_id=@order AND p.idempotency_key=@key FOR UPDATE OF p",tx)){Add(existing,"tenant",tenant);Add(existing,"branch",branch);Add(existing,"order",orderId);Add(existing,"key",idempotencyKey);await using var er=await existing.ExecuteReaderAsync(ct);if(await er.ReadAsync(ct)){var replay=new PaymentResponse(er.GetGuid(0),orderId,er.GetString(2),er.GetDecimal(1),0,request.Splits,true,er.GetString(3),Math.Max(0,er.GetDecimal(4)));await er.DisposeAsync();await tx.CommitAsync(ct);return replay;}}
      ...
      var payment=Guid.NewGuid();await using(var cmd=Command(c,"INSERT INTO barber.payments(id,tenant_id,branch_id,service_order_id,cash_register_id,idempotency_key,method,amount,paid_at,status,payload) VALUES(@id,@tenant,@branch,@order,@register,@key,@method,@amount,now(),'Paid',jsonb_build_object('note',@note))",tx)){Add(cmd,"id",payment);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);Add(cmd,"order",orderId);Add(cmd,"register",register);Add(cmd,"key",idempotencyKey);Add(cmd,"method",request.Splits.Count>1?"Mixed":request.Splits[0].Method);Add(cmd,"amount",amount);Add(cmd,"note",request.Note);await cmd.ExecuteNonQueryAsync(ct);}foreach(var split in request.Splits){await using var sc=Command(c,"INSERT INTO barber.payment_splits(id,payment_id,method,amount) VALUES(@id,@payment,@method,@amount)",tx);Add(sc,"id",Guid.NewGuid());Add(sc,"payment",payment);Add(sc,"method",split.Method);Add(sc,"amount",split.Amount);await sc.ExecuteNonQueryAsync(ct);}if(cash>0){await using var cc=Command(c,"INSERT INTO barber.cash_movements(id,tenant_id,branch_id,cash_register_id,payment_id,type,amount,description,origin,created_by) VALUES(@id,@tenant,@branch,@register,@payment,'Sale',@amount,'Recebimento de comanda','POS',@user)",tx);Add(cc,"id",Guid.NewGuid());Add(cc,"tenant",tenant);Add(cc,"branch",branch);Add(cc,"register",register);Add(cc,"payment",payment);Add(cc,"amount",cash);Add(cc,"user",user);await cc.ExecuteNonQueryAsync(ct);}
      if(orderStatus=="Paid")await PostSale(c,tx,tenant,branch,user,orderId,payment,ct);await using(var close=Command(c,"UPDATE barber.service_orders SET status=@status,updated_at=now() WHERE id=@id AND tenant_id=@tenant AND branch_id=@branch AND status IN ('Open','PartiallyPaid')",tx)){Add(close,"id",orderId);Add(close,"tenant",tenant);Add(close,"branch",branch);Add(close,"status",orderStatus);if(await close.ExecuteNonQueryAsync(ct)!=1)throw new InvalidOperationException("A comanda foi alterada por outra operação.");}await Audit(c,tx,tenant,branch,user,"Payment.Register",payment,orderStatus=="Paid"?"Pagamento confirmado":"Pagamento parcial confirmado",ct);await tx.CommitAsync(ct);var received=request.Splits.Where(x=>x.Method.Equals("Cash",StringComparison.OrdinalIgnoreCase)).Sum(x=>x.ReceivedAmount??x.Amount);return new(payment,orderId,"Paid",amount,Math.Max(0,received-cash),request.Splits,false,orderStatus,Math.Max(0,order.Balance-amount)); }
    ...
    { if(string.IsNullOrWhiteSpace(request.Reason))throw new InvalidOperationException("Informe o motivo do estorno.");await using var c=await connections.OpenConnectionAsync(ct);await using var tx=await c.BeginTransactionAsync(ct);Guid order;decimal amount;await using(var cmd=Command(c,"UPDATE barber.payments SET status='Refunded',updated_at=now() WHERE id=@id AND tenant_id=@tenant AND branch_id=@branch AND status='Paid' RETURNING service_order_id,amount",tx)){Add(cmd,"id",paymentId);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);await using var r=await cmd.ExecuteReaderAsync(ct);if(!await r.ReadAsync(ct))throw new InvalidOperationException("Pagamento não encontrado ou já estornado.");order=r.GetGuid(0);amount=r.GetDecimal(1);}
      await using(var cash=Command(c,"INSERT INTO barber.cash_movements(id,tenant_id,branch_id,cash_register_id,payment_id,type,amount,description,origin,created_by) SELECT gen_random_uuid(),tenant_id,branch_id,cash_register_id,id,'Refund',-amount,@reason,'PaymentRefund',@user FROM barber.payments WHERE id=@payment AND cash_register_id IS NOT NULL",tx)){Add(cash,"payment",paymentId);Add(cash,"reason",request.Reason);Add(cash,"user",user);await cash.ExecuteNonQueryAsync(ct);}
    ...
    { int expectedProducts;await using(var expected=Command(c,"SELECT count(DISTINCT product_id) FROM barber.service_order_items WHERE service_order_id=@order AND tenant_id=@tenant AND branch_id=@branch AND product_id IS NOT NULL AND deleted_at IS NULL",tx)){Add(expected,"order",order);Add(expected,"tenant",tenant);Add(expected,"branch",branch);expectedProducts=Convert.ToInt32(await expected.ExecuteScalarAsync(ct));}await using(var stock=Command(c,"WITH sold AS (SELECT i.product_id,sum(i.quantity) qty FROM barber.service_order_items i WHERE i.service_order_id=@order AND i.tenant_id=@tenant AND i.branch_id=@branch AND i.product_id IS NOT NULL AND i.deleted_at IS NULL GROUP BY i.product_id), upd AS (UPDATE barber.products p SET current_stock=p.current_stock-s.qty,updated_at=now() FROM sold s WHERE p.id=s.product_id AND p.tenant_id=@tenant AND p.branch_id=@branch AND (p.allow_negative_stock OR p.current_stock>=s.qty) RETURNING p.id,p.current_stock AS balance_after,s.qty) INSERT INTO barber.stock_movements(id,tenant_id,branch_id,product_id,service_order_id,type,quantity,balance_after,reason,created_by) SELECT gen_random_uuid(),@tenant,@branch,id,@order,'Sale',qty,balance_after,'Venda em comanda',@user FROM upd",tx)){Add(stock,"order",order);Add(stock,"tenant",tenant);Add(stock,"branch",branch);Add(stock,"user",user);var movedProducts=await stock.ExecuteNonQueryAsync(ct);if(movedProducts!=expectedProducts)throw new InvalidOperationException("Estoque insuficiente para concluir o pagamento.");}
      const string recipeSql="SELECT count(DISTINCT ri.product_id) FROM barber.service_order_items oi JOIN barber.service_input_recipes r ON r.service_id=oi.service_id AND r.tenant_id=@tenant AND r.branch_id=@branch AND r.status='Active' AND r.deleted_at IS NULL JOIN barber.service_input_recipe_items ri ON ri.recipe_id=r.id WHERE oi.service_order_id=@order AND oi.service_id IS NOT NULL AND oi.deleted_at IS NULL AND NOT EXISTS(SELECT 1 FROM barber.service_order_items explicit_item WHERE explicit_item.service_order_id=@order AND explicit_item.product_id=ri.product_id AND explicit_item.deleted_at IS NULL) AND NOT EXISTS(SELECT 1 FROM barber.stock_movements m WHERE m.tenant_id=@tenant AND m.branch_id=@branch AND m.service_order_id=@order AND m.product_id=ri.product_id AND m.type='ServiceConsumption')";
      int expectedInputs;await using(var countInputs=Command(c,recipeSql,tx)){Add(countInputs,"order",order);Add(countInputs,"tenant",tenant);Add(countInputs,"branch",branch);expectedInputs=Convert.ToInt32(await countInputs.ExecuteScalarAsync(ct));}
      await using(var inputs=Command(c,@"WITH required AS (SELECT ri.product_id,sum(ri.quantity*oi.quantity) qty FROM barber.service_order_items oi JOIN barber.service_input_recipes r ON r.service_id=oi.service_id AND r.tenant_id=@tenant AND r.branch_id=@branch AND r.status='Active' AND r.deleted_at IS NULL JOIN barber.service_input_recipe_items ri ON ri.recipe_id=r.id WHERE oi.service_order_id=@order AND oi.service_id IS NOT NULL AND oi.deleted_at IS NULL AND NOT EXISTS(SELECT 1 FROM barber.service_order_items explicit_item WHERE explicit_item.service_order_id=@order AND explicit_item.product_id=ri.product_id AND explicit_item.deleted_at IS NULL) GROUP BY ri.product_id), updated AS (UPDATE barber.products p SET current_stock=p.current_stock-r.qty,updated_at=now() FROM required r WHERE p.id=r.product_id AND p.tenant_id=@tenant AND p.branch_id=@branch AND (p.allow_negative_stock OR p.current_stock>=r.qty) RETURNING p.id,p.current_stock balance_after,r.qty) INSERT INTO barber.stock_movements(id,tenant_id,branch_id,product_id,service_order_id,type,quantity,balance_after,reason,created_by) SELECT gen_random_uuid(),@tenant,@branch,id,@order,'ServiceConsumption',-qty,balance_after,'Baixa automática da ficha técnica',@user FROM updated WHERE NOT EXISTS(SELECT 1 FROM barber.stock_movements m WHERE m.tenant_id=@tenant AND m.branch_id=@branch AND m.service_order_id=@order AND m.product_id=updated.id AND m.type='ServiceConsumption')",tx)){Add(inputs,"order",order);Add(inputs,"tenant",tenant);Add(inputs,"branch",branch);Add(inputs,"user",user);if(await inputs.ExecuteNonQueryAsync(ct)!=expectedInputs)throw new InvalidOperationException("Estoque de insumos insuficiente para concluir o pagamento.");}
      await using(var comm=Command(c,"""
    ...
    { string number,status;Guid client;Guid? appointment;decimal subtotal,discount,surcharge,total,paid;await using(var cmd=Command(c,"SELECT o.number,o.client_id,o.appointment_id,o.status,o.subtotal,o.discount,o.surcharge,o.total,COALESCE((SELECT sum(amount) FROM barber.payments WHERE service_order_id=o.id AND tenant_id=@tenant AND branch_id=@branch AND status='Paid'),0) FROM barber.service_orders o WHERE o.id=@id AND o.tenant_id=@tenant AND o.branch_id=@branch AND o.deleted_at IS NULL",tx)){Add(cmd,"id",id);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);await using var r=await cmd.ExecuteReaderAsync(ct);if(!await r.ReadAsync(ct))return null;number=r.IsDBNull(0)?id.ToString("N")[..8]:r.GetString(0);client=r.GetGuid(1);appointment=r.IsDBNull(2)?null:r.GetGuid(2);status=r.GetString(3);subtotal=r.GetDecimal(4);discount=r.GetDecimal(5);surcharge=r.GetDecimal(6);total=r.GetDecimal(7);paid=r.GetDecimal(8);}var items=new List<ServiceOrderItemResponse>();await using(var cmd=Command(c,"SELECT id,item_type,service_id,product_id,professional_id,description,quantity,unit_price,discount,total FROM barber.service_order_items WHERE service_order_id=@id AND tenant_id=@tenant AND branch_id=@branch AND deleted_at IS NULL ORDER BY created_at",tx)){Add(cmd,"id",id);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct))items.Add(new(r.GetGuid(0),r.GetString(1),r.IsDBNull(2)?null:r.GetGuid(2),r.IsDBNull(3)?null:r.GetGuid(3),r.IsDBNull(4)?null:r.GetGuid(4),r.IsDBNull(5)?"Item":r.GetString(5),r.GetDecimal(6),r.GetDecimal(7),r.GetDecimal(8),r.GetDecimal(9)));}var payments=new List<ServiceOrderPaymentResponse>();await using(var cmd=Command(c,"SELECT id,method,amount,status,paid_at FROM barber.payments WHERE service_order_id=@id AND tenant_id=@tenant AND branch_id=@branch ORDER BY created_at",tx)){Add(cmd,"id",id);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct))payments.Add(new(r.GetGuid(0),r.GetString(1),r.GetDecimal(2),r.GetString(3),r.IsDBNull(4)?null:new DateTimeOffset(r.GetDateTime(4),TimeSpan.Zero)));}return new(id,number,client,appointment,status,subtotal,discount,surcharge,total,paid,Math.Max(0,total-paid),items,payments); }
    ...
    private static async Task Recalculate(DbConnection c,DbTransaction tx,Guid tenant,Guid branch,Guid id,CancellationToken ct){await using var cmd=Command(c,"UPDATE barber.service_orders SET subtotal=COALESCE((SELECT sum(total) FROM barber.service_order_items WHERE service_order_id=@id AND tenant_id=@tenant AND branch_id=@branch AND deleted_at IS NULL),0),total=greatest(0,COALESCE((SELECT sum(total) FROM barber.service_order_items WHERE service_order_id=@id AND tenant_id=@tenant AND branch_id=@branch AND deleted_at IS NULL),0)-discount+surcharge),updated_at=now() WHERE id=@id AND tenant_id=@tenant AND branch_id=@branch",tx);Add(cmd,"id",id);Add(cmd,"tenant",tenant);Add(cmd,"branch",branch);await cmd.ExecuteNonQueryAsync(ct);}
