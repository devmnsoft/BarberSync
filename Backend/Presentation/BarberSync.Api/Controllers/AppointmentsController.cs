...
public sealed class AppointmentsController(IAppointmentService appointments) : ControllerBase
{
    ...
    [HttpPost("{id:guid}/cancel"), RequirePermission("Appointment.Cancel")] public Task<AppointmentResponse> Cancel(Guid id,CancelAppointmentRequest request,CancellationToken ct)=>appointments.ChangeStatusAsync(id,"Cancelled",request.Reason,ct);
    [HttpPost("{id:guid}/no-show"), RequirePermission("Appointment.Update")] public Task<AppointmentResponse> NoShow(Guid id,NoShowAppointmentRequest request,CancellationToken ct)=>appointments.ChangeStatusAsync(id,"NoShow",request.Reason,ct);
