...
public sealed class ExecutiveController(ExecutiveInsightsService insights, ILogger<ExecutiveController> logger) : ControllerBase
{
    ...
    public Task<IActionResult> Owner([FromQuery] DateOnly? from, [FromQuery] DateOnly? to, CancellationToken ct) => Execute(() => insights.OwnerAsync(from, to, ct));
    ...
    public async Task<IActionResult> Csv([FromQuery] DateOnly? from, [FromQuery] DateOnly? to, CancellationToken ct)
    {
        var data = await insights.OwnerAsync(from, to, ct); await insights.AuditAsync("ReportExportedCsv", $"Dashboard executivo exportado em CSV. Período: {from:yyyy-MM-dd} a {to:yyyy-MM-dd}.", ct);
    ...
    private async Task<IActionResult> Execute(Func<Task<object>> action)
    {
        ...
        catch (ArgumentException exception) { return BadRequest(new ProblemDetails { Title = "Período inválido", Detail = exception.Message, Extensions = { ["traceId"] = HttpContext.TraceIdentifier } }); }
