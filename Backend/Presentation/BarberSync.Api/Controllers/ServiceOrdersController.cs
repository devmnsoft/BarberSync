...
public sealed class ServiceOrdersController(EnterpriseDataService data, ILogger<ServiceOrdersController> logger) : EnterpriseCrudController(data, logger, "service-orders")
{
    ...
    [HttpPost("{id:guid}/pay")] public Task<IActionResult> Pay(Guid id, [FromBody] JsonElement payload, CancellationToken cancellationToken) => Safe(async () => Ok(Envelope(await data.CreateAsync("payments", payload, cancellationToken), "Pagamento registrado com sucesso.")));
