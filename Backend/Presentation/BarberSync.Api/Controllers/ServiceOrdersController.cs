...
public sealed class ServiceOrdersController(IServiceOrderService orders, IPaymentService payments) : ControllerBase
{
    ...
    [HttpDelete("{id:guid}/items/{itemId:guid}"), RequirePermission("ServiceOrder.Update")] public Task<ServiceOrderResponse> RemoveItem(Guid id,Guid itemId,RemoveOrderItemRequest request,CancellationToken ct) => orders.RemoveItemAsync(id,itemId,request,ct);
