...
public sealed class StockController(EnterpriseDataService data, ILogger<StockController> logger) : EnterpriseCrudController(data, logger, "products")
{
    ...
    [HttpGet("movements"), RequirePermission("Stock.View")] public Task<IActionResult> Movements(CancellationToken cancellationToken) => Safe(async () => Ok(Envelope(await data.ListAsync("stock_movements", cancellationToken), "Movimentos de estoque carregados com sucesso.")));
