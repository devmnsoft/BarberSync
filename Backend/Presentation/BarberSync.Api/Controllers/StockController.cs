...
public sealed class StockController(EnterpriseDataService data, ILogger<StockController> logger) : EnterpriseCrudController(data, logger, "products")
{
    ...
    [HttpGet("critical")] public Task<IActionResult> Critical(CancellationToken cancellationToken) => Safe(async () => Ok(Envelope(await data.CriticalStockAsync(cancellationToken), "Estoque crítico carregado com sucesso.")));
