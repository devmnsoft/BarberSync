...
from barber.clients c where c.id=@client and c.tenant_id=@tenant and c.branch_id=@branch and c.deleted_at is null", c => c.Parameters.AddWithValue("client", clientId), ct);
    ...
    public Task<JsonElement> Timeline(Guid clientId, CancellationToken ct) => Json("select coalesce(jsonb_agg(to_jsonb(e) order by occurred_at desc,created_at desc),'[]') from barber.client_timeline_events e where tenant_id=@tenant and branch_id=@branch and client_id=@client", c => c.Parameters.AddWithValue("client", clientId), ct);
