...
public sealed partial class EnterpriseDataService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
{
    ...
    public async Task<IReadOnlyList<Dictionary<string, object?>>> ListAsync(string resource, CancellationToken cancellationToken = default)
    {
        ...
        var projection = resource.ToLowerInvariant() switch
        {
            ...
            "coupons" => "jsonb_strip_nulls(jsonb_build_object('id',id::text,'tenantId',tenant_id::text,'branchId',branch_id::text,'code',code,'name',name,'discountType',discount_type,'discountValue',discount_value,'startsAt',valid_from,'endsAt',valid_until,'status',status,'isActive',is_active,'createdAt',created_at)||payload)",
            "loyalty_accounts" or "loyalty" => "jsonb_strip_nulls(jsonb_build_object('id',id::text,'tenantId',tenant_id::text,'branchId',branch_id::text,'clientId',client_id::text,'pointsBalance',points_balance,'cashbackBalance',cashback_balance,'lifetimePoints',lifetime_points,'lifetimeCashback',lifetime_cashback,'status',status,'isActive',is_active,'createdAt',created_at)||payload)",
