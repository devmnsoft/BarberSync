...
public interface IServiceRecognitionRepository
{
    ...
    Task RecordProviderStatusAsync(Guid tenant, Guid branch, Guid user, string status, string traceId, CancellationToken ct);
}
...
public sealed class ServiceRecognitionService(IDbConnectionFactory connections, IAiProvider provider) : IServiceRecognitionService
{
    ...
    public async Task RecordProviderStatusAsync(Guid tenant,Guid branch,Guid user,string status,string trace,CancellationToken ct)
    {
        var available=string.Equals(status,"Connected",StringComparison.OrdinalIgnoreCase);
        var action=available?"AiProviderTested":"AiProviderUnavailable";
        var description=available?"Provider de IA testado com sucesso.":"Provider de IA não configurado ou indisponível.";
        await using var db=await connections.OpenConnectionAsync(ct);await using var tx=await db.BeginTransactionAsync(ct);
        await Audit(db,tx,tenant,branch,user,action,Guid.Empty,trace,description,ct);
        if(!available)
        {
            var type=string.Equals(status,"NotConfigured",StringComparison.OrdinalIgnoreCase)?"AiProviderNotConfigured":"AiProviderUnavailable";
            var title=type=="AiProviderNotConfigured"?"Provider de IA não configurado":"Erro no provider de IA";
            await Notify(db,tx,tenant,branch,type,title,"/Admin/System/AiSettings",Guid.Empty,ct);
        }
