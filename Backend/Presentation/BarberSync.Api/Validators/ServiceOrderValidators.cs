...
{ public RegisterPaymentRequestValidator() { RuleFor(x=>x.IdempotencyKey).NotEmpty().MaximumLength(100); RuleFor(x=>x.Splits).NotEmpty(); RuleFor(x=>x.Note).MaximumLength(500); RuleForEach(x=>x.Splits).ChildRules(s=>{s.RuleFor(x=>x.Method).Must(x=>new[]{"Cash","Pix","DebitCard","CreditCard"}.Contains(x,StringComparer.OrdinalIgnoreCase)).WithMessage("Forma de pagamento inválida.");s.RuleFor(x=>x.Amount).GreaterThan(0);s.RuleFor(x=>x).Must(x=>x.Method.Equals("Cash",StringComparison.OrdinalIgnoreCase)||x.ReceivedAmount is null).WithMessage("Valor recebido é exclusivo de pagamentos em dinheiro.");s.RuleFor(x=>x).Must(x=>x.ReceivedAmount is null||x.ReceivedAmount>=x.Amount).WithMessage("Valor recebido não pode ser menor que o valor aplicado.");}); } }
