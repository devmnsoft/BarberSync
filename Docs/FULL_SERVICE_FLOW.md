...
## Estabilização de proxies e endpoints demo

Para a demonstração comercial, os navegadores continuam usando apenas proxies relativos:

- AdminWeb: `/AdminApi/...`
- PublicWeb: `/PublicApi/...`
- KioskWeb: `/KioskApi/...`

A URL interna da API fica restrita ao Docker/server-side em `ApiSettings:BaseUrl`. Os endpoints demo expostos pela API para reduzir fallbacks e manter logs limpos incluem:

- `GET /api/kiosk/status`, `GET /api/kiosk/services`, `GET /api/kiosk/professionals`
- `POST /api/kiosk/client/quick-register`, `POST /api/kiosk/payment/mock`, `POST /api/kiosk/review`
- `POST /api/appointments/{id}/confirm|check-in|start|finish|cancel`
- `POST /api/service-orders/{id}/pay|close`
- `GET /api/loyalty/summary`, `GET /api/financial/summary`, `GET /api/reports/summary`, `GET /api/copilot/suggestions`
- `POST /api/leads`

Esses endpoints retornam HTTP 200 com payloads demo quando usados pelos proxies MVC, permitindo que CRUDs, dashboard, totem, site público e fluxo completo continuem apresentáveis mesmo sem banco produtivo populado.
