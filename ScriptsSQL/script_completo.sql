...
-- Agenda Inteligente Premium (Sprint 48). Todas as estruturas são aditivas e escopadas.
INSERT INTO barber.permissions(id,code,description) VALUES
(gen_random_uuid(),'Scheduling.Read','Consultar agenda'),(gen_random_uuid(),'Scheduling.Manage','Gerenciar agenda'),
(gen_random_uuid(),'Scheduling.Book','Criar agendamentos'),(gen_random_uuid(),'Scheduling.Cancel','Cancelar agendamentos'),
(gen_random_uuid(),'Scheduling.Reschedule','Reagendar atendimentos'),(gen_random_uuid(),'Scheduling.Waitlist','Gerenciar lista de espera'),
(gen_random_uuid(),'Scheduling.Settings','Configurar políticas e recursos') ON CONFLICT(code) DO UPDATE SET description=excluded.description;
CREATE TABLE IF NOT EXISTS barber.scheduling_policies(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),name varchar(120) NOT NULL,min_advance_minutes integer NOT NULL DEFAULT 60,max_advance_days integer NOT NULL DEFAULT 60,cancellation_limit_minutes integer NOT NULL DEFAULT 120,reschedule_limit_minutes integer NOT NULL DEFAULT 120,buffer_minutes integer NOT NULL DEFAULT 0,allow_waitlist boolean NOT NULL DEFAULT true,allow_online_booking boolean NOT NULL DEFAULT true,require_confirmation boolean NOT NULL DEFAULT true,require_deposit boolean NOT NULL DEFAULT false,require_cancellation_reason boolean NOT NULL DEFAULT true,no_show_policy_json jsonb NOT NULL DEFAULT '{}'::jsonb,status varchar(20) NOT NULL DEFAULT 'Active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz);
CREATE UNIQUE INDEX IF NOT EXISTS ux_scheduling_policies_scope ON barber.scheduling_policies(tenant_id,branch_id) WHERE deleted_at IS NULL AND status='Active';
CREATE INDEX IF NOT EXISTS ix_scheduling_policies_status ON barber.scheduling_policies(tenant_id,branch_id,status);
CREATE TABLE IF NOT EXISTS barber.scheduling_resources(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),name varchar(120) NOT NULL,resource_type varchar(30) NOT NULL,capacity integer NOT NULL DEFAULT 1,status varchar(20) NOT NULL DEFAULT 'Active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz,CHECK(resource_type IN ('Chair','Room','WashBasin','Equipment','Other')),CHECK(capacity>0));
CREATE INDEX IF NOT EXISTS ix_scheduling_resources_scope ON barber.scheduling_resources(tenant_id,branch_id,status,resource_type);
CREATE TABLE IF NOT EXISTS barber.appointment_services(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),appointment_id uuid NOT NULL REFERENCES barber.appointments(id),service_id uuid NOT NULL REFERENCES barber.services(id),professional_id uuid NOT NULL REFERENCES barber.professionals(id),resource_id uuid REFERENCES barber.scheduling_resources(id),duration_minutes integer NOT NULL,price_snapshot numeric(12,2),status varchar(20) NOT NULL DEFAULT 'Active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,CHECK(duration_minutes>0));
CREATE INDEX IF NOT EXISTS ix_appointment_services_scope ON barber.appointment_services(tenant_id,branch_id,appointment_id,professional_id,status);
CREATE INDEX IF NOT EXISTS ix_appointment_services_resource ON barber.appointment_services(tenant_id,branch_id,resource_id,status);
CREATE TABLE IF NOT EXISTS barber.appointment_waitlist(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),service_id uuid NOT NULL REFERENCES barber.services(id),preferred_professional_id uuid REFERENCES barber.professionals(id),preferred_date date NOT NULL,preferred_period varchar(20) NOT NULL DEFAULT 'Any',status varchar(20) NOT NULL DEFAULT 'Waiting',notes text,cancellation_reason text,offered_start_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,expires_at timestamptz,CHECK(preferred_period IN ('Morning','Afternoon','Evening','Any')),CHECK(status IN ('Waiting','Offered','Booked','Cancelled','Expired')));
CREATE INDEX IF NOT EXISTS ix_appointment_waitlist_scope ON barber.appointment_waitlist(tenant_id,branch_id,status,preferred_date,preferred_professional_id);
CREATE TABLE IF NOT EXISTS barber.appointment_status_history(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),appointment_id uuid NOT NULL REFERENCES barber.appointments(id),old_status varchar(30),new_status varchar(30) NOT NULL,reason text,changed_by uuid REFERENCES barber.users(id),changed_at timestamptz NOT NULL DEFAULT now(),metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb);
CREATE INDEX IF NOT EXISTS ix_appointment_status_history_scope ON barber.appointment_status_history(tenant_id,branch_id,appointment_id,changed_at);
CREATE TABLE IF NOT EXISTS barber.appointment_confirmations(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),appointment_id uuid NOT NULL REFERENCES barber.appointments(id),channel_type varchar(20) NOT NULL,status varchar(20) NOT NULL DEFAULT 'Pending',confirmed_at timestamptz,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,CHECK(channel_type IN ('InApp','Email','Sms','WhatsApp','Manual')),CHECK(status IN ('Pending','Confirmed','Failed','Expired')));
CREATE INDEX IF NOT EXISTS ix_appointment_confirmations_scope ON barber.appointment_confirmations(tenant_id,branch_id,appointment_id,status);
CREATE TABLE IF NOT EXISTS barber.appointment_reschedule_requests(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),appointment_id uuid NOT NULL REFERENCES barber.appointments(id),requested_by uuid REFERENCES barber.users(id),old_start_at timestamptz NOT NULL,old_end_at timestamptz NOT NULL,new_start_at timestamptz NOT NULL,new_end_at timestamptz NOT NULL,reason text NOT NULL,status varchar(20) NOT NULL DEFAULT 'Pending',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,CHECK(status IN ('Pending','Approved','Rejected','Cancelled')),CHECK(new_end_at>new_start_at));
...
INSERT INTO barber.permissions(id,code,description) VALUES
(gen_random_uuid(),'Clients360.Read','Consultar Cliente 360'),(gen_random_uuid(),'Clients360.Manage','Gerenciar Cliente 360'),
(gen_random_uuid(),'Clients360.Sensitive.Read','Consultar dados sensíveis do Cliente 360'),(gen_random_uuid(),'Clients360.VisualRecords.Manage','Gerenciar histórico visual'),
(gen_random_uuid(),'Clients360.Consents.Manage','Gerenciar consentimentos'),(gen_random_uuid(),'Clients360.Budgets.Manage','Gerenciar orçamentos'),
(gen_random_uuid(),'Clients360.TreatmentPlans.Manage','Gerenciar planos de tratamento'),(gen_random_uuid(),'Clients360.Export','Exportar Cliente 360')
ON CONFLICT(code) DO UPDATE SET description=excluded.description;

CREATE TABLE IF NOT EXISTS barber.client_technical_sheets(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),service_order_id uuid REFERENCES barber.service_orders(id),appointment_id uuid REFERENCES barber.appointments(id),sheet_type varchar(20) NOT NULL,title varchar(160) NOT NULL,summary text,technical_notes text,formula_json jsonb NOT NULL DEFAULT '{}'::jsonb,measurements_json jsonb NOT NULL DEFAULT '{}'::jsonb,risk_flags_json jsonb NOT NULL DEFAULT '[]'::jsonb,status varchar(20) NOT NULL DEFAULT 'Draft',created_by uuid NOT NULL REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,archived_at timestamptz,CHECK(sheet_type IN ('Hair','Beard','Nails','Brows','Aesthetic','Makeup','Other')),CHECK(status IN ('Draft','Active','Archived')));
CREATE INDEX IF NOT EXISTS ix_client_technical_sheets_scope ON barber.client_technical_sheets(tenant_id,branch_id,client_id,status,created_at DESC);
CREATE INDEX IF NOT EXISTS ix_client_technical_sheets_professional ON barber.client_technical_sheets(tenant_id,branch_id,professional_id,created_at DESC);
CREATE TABLE IF NOT EXISTS barber.client_anamnesis_forms(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),appointment_id uuid REFERENCES barber.appointments(id),service_order_id uuid REFERENCES barber.service_orders(id),form_type varchar(20) NOT NULL,status varchar(20) NOT NULL DEFAULT 'Draft',risk_level varchar(10) NOT NULL DEFAULT 'Low',review_notes text,created_by uuid NOT NULL REFERENCES barber.users(id),reviewed_by uuid REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),reviewed_at timestamptz,updated_at timestamptz,CHECK(form_type IN ('General','Aesthetic','Chemical','Hair','Beard','Nails','Brows')),CHECK(status IN ('Draft','Completed','Reviewed','Archived')),CHECK(risk_level IN ('Low','Medium','High')));
CREATE INDEX IF NOT EXISTS ix_client_anamnesis_scope ON barber.client_anamnesis_forms(tenant_id,branch_id,client_id,status,risk_level,created_at DESC);
CREATE TABLE IF NOT EXISTS barber.client_anamnesis_answers(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),anamnesis_id uuid NOT NULL REFERENCES barber.client_anamnesis_forms(id),question_key varchar(100) NOT NULL,question_text text NOT NULL,answer_type varchar(20) NOT NULL,answer_value text,answer_json jsonb,is_sensitive boolean NOT NULL DEFAULT false,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,UNIQUE(anamnesis_id,question_key));
CREATE INDEX IF NOT EXISTS ix_client_anamnesis_answers_scope ON barber.client_anamnesis_answers(tenant_id,branch_id,anamnesis_id,created_at);
CREATE TABLE IF NOT EXISTS barber.client_consent_terms(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),code varchar(80) NOT NULL,title varchar(180) NOT NULL,body text NOT NULL,version varchar(30) NOT NULL,status varchar(20) NOT NULL DEFAULT 'Draft',created_by uuid NOT NULL REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz,CHECK(status IN ('Draft','Active','Inactive')));
CREATE UNIQUE INDEX IF NOT EXISTS ux_client_consent_terms_version ON barber.client_consent_terms(tenant_id,branch_id,code,version) WHERE deleted_at IS NULL;
CREATE TABLE IF NOT EXISTS barber.client_consent_acceptances(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),consent_term_id uuid NOT NULL REFERENCES barber.client_consent_terms(id),accepted_by uuid REFERENCES barber.users(id),acceptance_channel varchar(20) NOT NULL,accepted_at timestamptz NOT NULL DEFAULT now(),ip_address inet,user_agent text,signature_storage_key text,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,status varchar(20) NOT NULL DEFAULT 'Active',revoked_at timestamptz,revoked_reason text,created_at timestamptz NOT NULL DEFAULT now(),CHECK(status IN ('Active','Revoked')),CHECK(acceptance_channel IN ('Admin','Mobile','Kiosk','Public','Manual')));
CREATE INDEX IF NOT EXISTS ix_client_consent_acceptances_scope ON barber.client_consent_acceptances(tenant_id,branch_id,client_id,status,created_at DESC);
CREATE TABLE IF NOT EXISTS barber.client_visual_records(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),appointment_id uuid REFERENCES barber.appointments(id),service_order_id uuid REFERENCES barber.service_orders(id),record_type varchar(20) NOT NULL,title varchar(160) NOT NULL,description text,storage_key text NOT NULL,content_hash varchar(128) NOT NULL,mime_type varchar(100) NOT NULL,consent_acceptance_id uuid NOT NULL REFERENCES barber.client_consent_acceptances(id),privacy_status varchar(30) NOT NULL DEFAULT 'Private',created_by uuid NOT NULL REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,archived_at timestamptz,archive_reason text,CHECK(record_type IN ('Before','After','Progress','Reference','Other')),CHECK(privacy_status IN ('Private','SharedWithClient','Archived')));
CREATE INDEX IF NOT EXISTS ix_client_visual_records_scope ON barber.client_visual_records(tenant_id,branch_id,client_id,privacy_status,created_at DESC);
CREATE TABLE IF NOT EXISTS barber.client_budgets(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),title varchar(160) NOT NULL,description text,status varchar(20) NOT NULL DEFAULT 'Draft',valid_until date,subtotal numeric(14,2) NOT NULL DEFAULT 0,discount_total numeric(14,2) NOT NULL DEFAULT 0,total numeric(14,2) NOT NULL DEFAULT 0,created_by uuid NOT NULL REFERENCES barber.users(id),approved_at timestamptz,rejected_at timestamptz,rejection_reason text,converted_service_order_id uuid REFERENCES barber.service_orders(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,CHECK(status IN ('Draft','Presented','Approved','Rejected','Expired','Converted')),CHECK(total=subtotal-discount_total));
CREATE INDEX IF NOT EXISTS ix_client_budgets_scope ON barber.client_budgets(tenant_id,branch_id,client_id,status,created_at DESC);
CREATE TABLE IF NOT EXISTS barber.client_budget_items(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),budget_id uuid NOT NULL REFERENCES barber.client_budgets(id),service_id uuid REFERENCES barber.services(id),product_id uuid REFERENCES barber.products(id),description text NOT NULL,quantity numeric(12,3) NOT NULL,unit_price numeric(14,2) NOT NULL,discount_amount numeric(14,2) NOT NULL DEFAULT 0,total numeric(14,2) NOT NULL,sort_order integer NOT NULL DEFAULT 0,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,CHECK(service_id IS NOT NULL OR product_id IS NOT NULL),CHECK(total=quantity*unit_price-discount_amount));
CREATE INDEX IF NOT EXISTS ix_client_budget_items_scope ON barber.client_budget_items(tenant_id,branch_id,budget_id,sort_order);
CREATE TABLE IF NOT EXISTS barber.client_treatment_plans(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),title varchar(160) NOT NULL,objective text NOT NULL,status varchar(20) NOT NULL DEFAULT 'Draft',start_date date,end_date date,estimated_total numeric(14,2) NOT NULL DEFAULT 0,created_by uuid NOT NULL REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,completed_at timestamptz,cancelled_at timestamptz,cancel_reason text,CHECK(status IN ('Draft','Active','Paused','Completed','Cancelled')));
...
CREATE INDEX IF NOT EXISTS ix_client_preferences_scope ON barber.client_preferences(tenant_id,branch_id,client_id,created_at DESC);
CREATE TABLE IF NOT EXISTS barber.client_restrictions(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),restriction_type varchar(30) NOT NULL,title varchar(160) NOT NULL,description text NOT NULL,severity varchar(10) NOT NULL,status varchar(20) NOT NULL DEFAULT 'Active',created_by uuid NOT NULL REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,CHECK(restriction_type IN ('Allergy','ChemicalRestriction','MedicalNote','PersonalPreference','Other')),CHECK(severity IN ('Low','Medium','High','Critical')),CHECK(status IN ('Active','Inactive')));
CREATE INDEX IF NOT EXISTS ix_client_restrictions_scope ON barber.client_restrictions(tenant_id,branch_id,client_id,status,severity,created_at DESC);
CREATE TABLE IF NOT EXISTS barber.client_timeline_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),client_id uuid NOT NULL REFERENCES barber.clients(id),event_type varchar(60) NOT NULL,event_title varchar(180) NOT NULL,event_description text,source_type varchar(60),source_id uuid,occurred_at timestamptz NOT NULL DEFAULT now(),metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_client_timeline_scope ON barber.client_timeline_events(tenant_id,branch_id,client_id,occurred_at DESC,created_at DESC);
INSERT INTO barber.schema_versions(version,description,checksum) VALUES ('026','Cliente 360, prontuário, consentimentos, orçamentos e tratamentos','clients360-20260827') ON CONFLICT(version) DO UPDATE SET description=excluded.description,checksum=excluded.checksum;
...
-- Comunicação omnichannel (Sprint 40). Providers externos jamais são marcados como enviados sem confirmação real.
INSERT INTO barber.permissions(id,code,description) VALUES
('02200000-0000-4000-8000-000000000001','Communication.Read','Consultar comunicação'),('02200000-0000-4000-8000-000000000002','Communication.Manage','Gerenciar comunicação'),('02200000-0000-4000-8000-000000000003','Communication.Send','Enfileirar comunicações'),('02200000-0000-4000-8000-000000000004','Communication.Export','Exportar comunicação'),('02200000-0000-4000-8000-000000000005','Notification.Read','Consultar notificações'),('02200000-0000-4000-8000-000000000006','Notification.Manage','Gerenciar preferências') ON CONFLICT(code) DO UPDATE SET description=excluded.description;
CREATE TABLE IF NOT EXISTS barber.communication_channels(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),name varchar(120) NOT NULL,channel_type varchar(20) NOT NULL,provider_key varchar(80),status varchar(20) NOT NULL DEFAULT 'Active',settings_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_communication_channels_scope ON barber.communication_channels(tenant_id,branch_id,status,channel_type);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_communication_templates_code ON barber.communication_templates(tenant_id,branch_id,lower(code)) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_communication_templates_scope ON barber.communication_templates(tenant_id,branch_id,status,channel_type);
CREATE TABLE IF NOT EXISTS barber.communication_campaigns(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),name varchar(140) NOT NULL,description text,audience_type varchar(30) NOT NULL,segment_key varchar(80),template_id uuid NOT NULL REFERENCES barber.communication_templates(id),channel_type varchar(20) NOT NULL,status varchar(20) NOT NULL DEFAULT 'Draft',scheduled_at timestamptz,started_at timestamptz,completed_at timestamptz,created_by uuid REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,cancelled_at timestamptz,cancel_reason text,deleted_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_communication_campaigns_scope ON barber.communication_campaigns(tenant_id,branch_id,status,channel_type,scheduled_at);
CREATE TABLE IF NOT EXISTS barber.communication_campaign_recipients(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),campaign_id uuid NOT NULL REFERENCES barber.communication_campaigns(id),recipient_type varchar(20) NOT NULL,client_id uuid REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),user_id uuid REFERENCES barber.users(id),status varchar(20) NOT NULL DEFAULT 'Pending',skip_reason text,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_communication_recipients_scope ON barber.communication_campaign_recipients(tenant_id,branch_id,campaign_id,status);
CREATE TABLE IF NOT EXISTS barber.communication_automations(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),name varchar(140) NOT NULL,trigger_type varchar(40) NOT NULL,template_id uuid NOT NULL REFERENCES barber.communication_templates(id),channel_type varchar(20) NOT NULL,offset_minutes integer NOT NULL DEFAULT 0,conditions_json jsonb NOT NULL DEFAULT '{}'::jsonb,status varchar(20) NOT NULL DEFAULT 'Active',created_by uuid REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_communication_automations_scope ON barber.communication_automations(tenant_id,branch_id,status,channel_type,trigger_type);
CREATE TABLE IF NOT EXISTS barber.communication_outbox(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),channel_type varchar(20) NOT NULL,template_id uuid REFERENCES barber.communication_templates(id),campaign_id uuid REFERENCES barber.communication_campaigns(id),recipient_type varchar(20) NOT NULL,client_id uuid REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),user_id uuid REFERENCES barber.users(id),destination text,subject text,body text NOT NULL,payload_json jsonb NOT NULL DEFAULT '{}'::jsonb,status varchar(20) NOT NULL DEFAULT 'Pending',scheduled_at timestamptz NOT NULL DEFAULT now(),sent_at timestamptz,failed_at timestamptz,failure_reason text,idempotency_key varchar(180),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_communication_outbox_scope ON barber.communication_outbox(tenant_id,branch_id,status,channel_type,scheduled_at);
CREATE UNIQUE INDEX IF NOT EXISTS ux_communication_outbox_idempotency ON barber.communication_outbox(tenant_id,idempotency_key) WHERE idempotency_key IS NOT NULL;
CREATE TABLE IF NOT EXISTS barber.communication_delivery_attempts(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),outbox_id uuid NOT NULL REFERENCES barber.communication_outbox(id),attempt_number integer NOT NULL,provider_key varchar(80),status varchar(20) NOT NULL,provider_message_id varchar(180),response_json jsonb,error_message text,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(outbox_id,attempt_number));
CREATE INDEX IF NOT EXISTS ix_communication_attempts_scope ON barber.communication_delivery_attempts(tenant_id,branch_id,outbox_id,status);
CREATE TABLE IF NOT EXISTS barber.notification_preferences(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),recipient_type varchar(20) NOT NULL,client_id uuid REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),user_id uuid REFERENCES barber.users(id),channel_type varchar(20) NOT NULL,event_type varchar(50) NOT NULL,is_enabled boolean NOT NULL DEFAULT true,source varchar(40) NOT NULL DEFAULT 'SelfService',updated_at timestamptz,created_at timestamptz NOT NULL DEFAULT now());
CREATE UNIQUE INDEX IF NOT EXISTS ux_notification_preferences_user ON barber.notification_preferences(tenant_id,branch_id,user_id,channel_type,event_type) WHERE user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_notification_preferences_scope ON barber.notification_preferences(tenant_id,branch_id,channel_type,event_type,is_enabled);
CREATE TABLE IF NOT EXISTS barber.notification_inbox(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),recipient_type varchar(20) NOT NULL,client_id uuid REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),user_id uuid REFERENCES barber.users(id),title varchar(180) NOT NULL,message text NOT NULL,source_type varchar(40),source_id uuid,severity varchar(20) NOT NULL DEFAULT 'Info',status varchar(20) NOT NULL DEFAULT 'Unread',read_at timestamptz,created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_notification_inbox_scope ON barber.notification_inbox(tenant_id,branch_id,user_id,status,created_at);
CREATE TABLE IF NOT EXISTS barber.communication_suppression_list(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),recipient_type varchar(20) NOT NULL,client_id uuid REFERENCES barber.clients(id),professional_id uuid REFERENCES barber.professionals(id),destination text,channel_type varchar(20) NOT NULL,reason text NOT NULL,status varchar(20) NOT NULL DEFAULT 'Active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_communication_suppression_scope ON barber.communication_suppression_list(tenant_id,branch_id,status,channel_type);
CREATE TABLE IF NOT EXISTS barber.communication_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),event_type varchar(50) NOT NULL,source_type varchar(40) NOT NULL,source_id uuid,idempotency_key varchar(180) NOT NULL,payload_json jsonb NOT NULL DEFAULT '{}'::jsonb,status varchar(20) NOT NULL DEFAULT 'Pending',created_at timestamptz NOT NULL DEFAULT now(),processed_at timestamptz,UNIQUE(tenant_id,idempotency_key));
CREATE INDEX IF NOT EXISTS ix_communication_events_scope ON barber.communication_events(tenant_id,branch_id,status,event_type,created_at);
INSERT INTO barber.schema_versions(version,description,checksum) VALUES ('022','Comunicação omnichannel e notificações','communication-20260826') ON CONFLICT(version) DO UPDATE SET description=excluded.description,checksum=excluded.checksum;

-- IA Operacional (Sprint 41): apenas contexto operacional; revisão humana é obrigatória.
INSERT INTO barber.permissions(id,code,description) VALUES
('02300000-0000-4000-8000-000000000001','AiOperations.Read','Consultar IA operacional'),
('02300000-0000-4000-8000-000000000002','AiOperations.Manage','Gerenciar câmeras, zonas e regras'),
('02300000-0000-4000-8000-000000000003','AiOperations.Review','Revisar sugestões operacionais'),
('02300000-0000-4000-8000-000000000004','AiOperations.Export','Exportar relatórios de IA'),
('02300000-0000-4000-8000-000000000005','AiOperations.Settings','Configurar provider de IA')
ON CONFLICT(code) DO UPDATE SET description=excluded.description;
CREATE TABLE IF NOT EXISTS barber.ai_camera_devices(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),name varchar(120) NOT NULL,location_name varchar(160) NOT NULL,device_type varchar(30) NOT NULL,stream_url_masked text,status varchar(20) NOT NULL DEFAULT 'Inactive',privacy_mode varchar(30) NOT NULL DEFAULT 'MetadataOnly',settings_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz,CHECK(device_type IN ('Camera','TotemCamera','MobileCamera','ManualInput')),CHECK(privacy_mode IN ('MetadataOnly','BlurredSnapshot','NoImage')));
CREATE INDEX IF NOT EXISTS ix_ai_camera_devices_scope ON barber.ai_camera_devices(tenant_id,branch_id,status,created_at);
CREATE TABLE IF NOT EXISTS barber.ai_camera_zones(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),camera_device_id uuid NOT NULL REFERENCES barber.ai_camera_devices(id),name varchar(120) NOT NULL,zone_type varchar(30) NOT NULL,description text,status varchar(20) NOT NULL DEFAULT 'Active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_ai_camera_zones_scope ON barber.ai_camera_zones(tenant_id,branch_id,camera_device_id,status);
CREATE TABLE IF NOT EXISTS barber.ai_service_signal_rules(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),service_id uuid NOT NULL REFERENCES barber.services(id),name varchar(140) NOT NULL,description text,signals_json jsonb NOT NULL DEFAULT '[]'::jsonb,minimum_confidence numeric(5,4) NOT NULL DEFAULT .75,requires_human_review boolean NOT NULL DEFAULT true,status varchar(20) NOT NULL DEFAULT 'Active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz,CHECK(requires_human_review),CHECK(minimum_confidence BETWEEN 0 AND 1));
CREATE INDEX IF NOT EXISTS ix_ai_signal_rules_scope ON barber.ai_service_signal_rules(tenant_id,branch_id,status,service_id);
CREATE TABLE IF NOT EXISTS barber.ai_detection_sessions(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),camera_device_id uuid NOT NULL REFERENCES barber.ai_camera_devices(id),zone_id uuid REFERENCES barber.ai_camera_zones(id),professional_id uuid REFERENCES barber.professionals(id),appointment_id uuid REFERENCES barber.appointments(id),service_order_id uuid REFERENCES barber.service_orders(id),status varchar(20) NOT NULL DEFAULT 'Open',started_at timestamptz NOT NULL DEFAULT now(),ended_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_ai_detection_sessions_scope ON barber.ai_detection_sessions(tenant_id,branch_id,status,camera_device_id,professional_id);
CREATE TABLE IF NOT EXISTS barber.ai_detection_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),session_id uuid NOT NULL REFERENCES barber.ai_detection_sessions(id),camera_device_id uuid NOT NULL REFERENCES barber.ai_camera_devices(id),zone_id uuid REFERENCES barber.ai_camera_zones(id),event_type varchar(40) NOT NULL,confidence numeric(5,4) NOT NULL,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,occurred_at timestamptz NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),CHECK(confidence BETWEEN 0 AND 1));
CREATE INDEX IF NOT EXISTS ix_ai_detection_events_scope ON barber.ai_detection_events(tenant_id,branch_id,camera_device_id,event_type,occurred_at);
CREATE TABLE IF NOT EXISTS barber.ai_service_suggestions(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),session_id uuid NOT NULL REFERENCES barber.ai_detection_sessions(id),service_id uuid NOT NULL REFERENCES barber.services(id),professional_id uuid REFERENCES barber.professionals(id),appointment_id uuid REFERENCES barber.appointments(id),service_order_id uuid REFERENCES barber.service_orders(id),confidence numeric(5,4) NOT NULL,status varchar(20) NOT NULL DEFAULT 'PendingReview',reason text NOT NULL,expires_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,CHECK(status IN ('PendingReview','Approved','Rejected','Corrected','Expired','Cancelled')),CHECK(confidence BETWEEN 0 AND 1));
CREATE INDEX IF NOT EXISTS ix_ai_suggestions_scope ON barber.ai_service_suggestions(tenant_id,branch_id,status,service_id,professional_id,created_at);
CREATE TABLE IF NOT EXISTS barber.ai_suggestion_reviews(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),suggestion_id uuid NOT NULL REFERENCES barber.ai_service_suggestions(id),review_action varchar(20) NOT NULL,corrected_service_id uuid REFERENCES barber.services(id),service_order_id uuid REFERENCES barber.service_orders(id),review_reason text,reviewed_by uuid NOT NULL REFERENCES barber.users(id),reviewed_at timestamptz NOT NULL DEFAULT now(),created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(suggestion_id));
CREATE INDEX IF NOT EXISTS ix_ai_reviews_scope ON barber.ai_suggestion_reviews(tenant_id,branch_id,review_action,reviewed_at);
CREATE TABLE IF NOT EXISTS barber.ai_evidence_items(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),session_id uuid REFERENCES barber.ai_detection_sessions(id),suggestion_id uuid REFERENCES barber.ai_service_suggestions(id),evidence_type varchar(20) NOT NULL,storage_key text,content_hash varchar(128),redaction_status varchar(20) NOT NULL DEFAULT 'NotRequired',metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_ai_evidence_scope ON barber.ai_evidence_items(tenant_id,branch_id,suggestion_id,created_at);
CREATE TABLE IF NOT EXISTS barber.ai_provider_configs(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),provider_key varchar(80) NOT NULL,provider_type varchar(30) NOT NULL DEFAULT 'Disabled',status varchar(20) NOT NULL DEFAULT 'NotConfigured',settings_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz);
CREATE UNIQUE INDEX IF NOT EXISTS ux_ai_provider_scope ON barber.ai_provider_configs(tenant_id,branch_id) WHERE deleted_at IS NULL;
CREATE TABLE IF NOT EXISTS barber.ai_model_runs(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),provider_config_id uuid REFERENCES barber.ai_provider_configs(id),session_id uuid REFERENCES barber.ai_detection_sessions(id),status varchar(20) NOT NULL,input_summary_json jsonb NOT NULL DEFAULT '{}'::jsonb,output_summary_json jsonb NOT NULL DEFAULT '{}'::jsonb,error_message text,started_at timestamptz,completed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_ai_model_runs_scope ON barber.ai_model_runs(tenant_id,branch_id,status,created_at);
CREATE TABLE IF NOT EXISTS barber.ai_accuracy_snapshots(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),snapshot_date date NOT NULL,service_id uuid REFERENCES barber.services(id),total_suggestions integer NOT NULL DEFAULT 0,approved_count integer NOT NULL DEFAULT 0,rejected_count integer NOT NULL DEFAULT 0,corrected_count integer NOT NULL DEFAULT 0,average_confidence numeric(5,4),created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_ai_accuracy_scope ON barber.ai_accuracy_snapshots(tenant_id,branch_id,snapshot_date,service_id);
CREATE TABLE IF NOT EXISTS barber.ai_privacy_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),camera_device_id uuid REFERENCES barber.ai_camera_devices(id),event_type varchar(50) NOT NULL,details_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_by uuid REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_ai_privacy_scope ON barber.ai_privacy_events(tenant_id,branch_id,event_type,created_at);
INSERT INTO barber.schema_versions(version,description,checksum) VALUES ('023','IA Operacional com revisão humana e evidências minimizadas','ai-operations-20260826') ON CONFLICT(version) DO UPDATE SET description=excluded.description,checksum=excluded.checksum;
...
CREATE INDEX IF NOT EXISTS ix_saas_plans_status ON barber.saas_plans(status);
CREATE TABLE IF NOT EXISTS barber.saas_plan_features(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),plan_id uuid NOT NULL REFERENCES barber.saas_plans(id),feature_key varchar(80) NOT NULL,feature_name varchar(140) NOT NULL,is_enabled boolean NOT NULL DEFAULT false,limit_value integer,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,UNIQUE(plan_id,feature_key));
CREATE TABLE IF NOT EXISTS barber.tenant_subscriptions(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),plan_id uuid NOT NULL REFERENCES barber.saas_plans(id),status varchar(20) NOT NULL,billing_cycle varchar(20) NOT NULL,starts_at timestamptz NOT NULL,ends_at timestamptz,trial_ends_at timestamptz,cancelled_at timestamptz,cancel_reason text,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,CHECK(status IN ('Trial','Active','Suspended','Cancelled','Expired')),CHECK(billing_cycle IN ('Monthly','Annual')));
...
CREATE INDEX IF NOT EXISTS ix_user_invitations_scope ON barber.user_invitations(tenant_id,branch_id,status,email);
CREATE TABLE IF NOT EXISTS barber.custom_roles(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid REFERENCES barber.branches(id),code varchar(50) NOT NULL,name varchar(100) NOT NULL,description text,status varchar(20) NOT NULL DEFAULT 'Active',created_by uuid REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,deleted_at timestamptz);
CREATE UNIQUE INDEX IF NOT EXISTS ux_custom_roles_code ON barber.custom_roles(tenant_id,lower(code)) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_custom_roles_name ON barber.custom_roles(tenant_id,lower(name)) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_custom_roles_scope ON barber.custom_roles(tenant_id,branch_id,status);
CREATE TABLE IF NOT EXISTS barber.custom_role_permissions(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid REFERENCES barber.branches(id),role_id uuid NOT NULL REFERENCES barber.custom_roles(id),permission_code varchar(120) NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),created_by uuid REFERENCES barber.users(id),UNIQUE(role_id,permission_code));
CREATE INDEX IF NOT EXISTS ix_custom_role_permissions_scope ON barber.custom_role_permissions(tenant_id,branch_id,role_id);
CREATE TABLE IF NOT EXISTS barber.security_policies(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid REFERENCES barber.branches(id),policy_key varchar(80) NOT NULL,policy_value_json jsonb NOT NULL,status varchar(20) NOT NULL DEFAULT 'Active',updated_by uuid REFERENCES barber.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,UNIQUE(tenant_id,branch_id,policy_key));
...
CREATE TABLE IF NOT EXISTS barber.onboarding_checklists(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),name varchar(120) NOT NULL,status varchar(20) NOT NULL DEFAULT 'Active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,UNIQUE(tenant_id,branch_id,name));
CREATE INDEX IF NOT EXISTS ix_onboarding_checklists_scope ON barber.onboarding_checklists(tenant_id,branch_id,status);
CREATE TABLE IF NOT EXISTS barber.onboarding_checklist_items(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid NOT NULL REFERENCES barber.branches(id),checklist_id uuid NOT NULL REFERENCES barber.onboarding_checklists(id),item_key varchar(80) NOT NULL,title varchar(160) NOT NULL,description text,status varchar(20) NOT NULL DEFAULT 'Pending',skip_reason text,completed_by uuid REFERENCES barber.users(id),completed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz,UNIQUE(checklist_id,item_key));
CREATE INDEX IF NOT EXISTS ix_onboarding_items_scope ON barber.onboarding_checklist_items(tenant_id,branch_id,status,item_key);
CREATE TABLE IF NOT EXISTS barber.governance_audit_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid NOT NULL REFERENCES barber.tenants(id),branch_id uuid REFERENCES barber.branches(id),user_id uuid REFERENCES barber.users(id),module_key varchar(80) NOT NULL,action varchar(80) NOT NULL,severity varchar(20) NOT NULL DEFAULT 'Info',entity_type varchar(80),entity_id uuid,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,trace_id varchar(100),created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_governance_audit_scope ON barber.governance_audit_events(tenant_id,branch_id,user_id,module_key,severity,created_at);
INSERT INTO barber.schema_versions(version,description,checksum) VALUES ('024','Governança SaaS, planos, segurança, privacidade e onboarding','governance-20260826') ON CONFLICT(version) DO UPDATE SET description=excluded.description,checksum=excluded.checksum;
