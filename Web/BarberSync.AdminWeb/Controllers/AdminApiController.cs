...
public class AdminApiController(IHttpClientFactory httpClientFactory, IConfiguration configuration, ILogger<AdminApiController> logger) : ControllerBase
{
    ...
    [HttpGet("health/database")] public Task<IActionResult> DatabaseHealth() => ProxyGet("/api/health/database", new { success = false, databaseConnected = false, schemaReady = false, message = "Health database indisponível via proxy AdminApi.", isDemo = true }, "Health database indisponível via proxy AdminApi.");
    ...
    [HttpPost("appointments/{id}/no-show")] public Task<IActionResult> NoShowAppointment(string id) => ProxySend(HttpMethod.Post, $"/api/appointments/{Uri.EscapeDataString(id)}/no-show", null, DemoMutation("No-show registrado em modo demonstração.", id));

    [HttpPost("products")] public Task<IActionResult> CreateProduct([FromBody] JsonElement payload) => ProxySend(HttpMethod.Post, "/api/products", payload, DemoMutation("Produto criado em modo demonstração.", payload));
    ...
    [HttpPost("stock/adjustment")] public Task<IActionResult> StockAdjustment([FromBody] JsonElement payload) => ProxySend(HttpMethod.Post, "/api/stock/adjustment", payload, DemoMutation("Ajuste de estoque registrado em modo demonstração.", payload));
    ...
    [HttpPut("campaigns/{id}")] public Task<IActionResult> UpdateCampaign(string id, [FromBody] JsonElement payload) => ProxySend(HttpMethod.Put, $"/api/campaigns/{Uri.EscapeDataString(id)}", payload, DemoMutation("Campanha atualizada em modo demonstração.", payload, id));
    [HttpDelete("campaigns/{id}")] public Task<IActionResult> DeleteCampaign(string id) => ProxyDelete($"/api/campaigns/{Uri.EscapeDataString(id)}", DemoMutation("Campanha removida em modo demonstração.", id));
    ...
    [HttpPut("coupons/{id}")] public Task<IActionResult> UpdateCoupon(string id, [FromBody] JsonElement payload) => ProxySend(HttpMethod.Put, $"/api/coupons/{Uri.EscapeDataString(id)}", payload, DemoMutation("Cupom atualizado em modo demonstração.", payload, id));
    [HttpDelete("coupons/{id}")] public Task<IActionResult> DeleteCoupon(string id) => ProxyDelete($"/api/coupons/{Uri.EscapeDataString(id)}", DemoMutation("Cupom removido em modo demonstração.", id));
    [HttpPost("reviews")] public Task<IActionResult> CreateReview([FromBody] JsonElement payload) => ProxySend(HttpMethod.Post, "/api/reviews", payload, DemoMutation("Avaliação criada em modo demonstração.", payload));
    [HttpPut("reviews/{id}")] public Task<IActionResult> UpdateReview(string id, [FromBody] JsonElement payload) => ProxySend(HttpMethod.Put, $"/api/reviews/{Uri.EscapeDataString(id)}", payload, DemoMutation("Avaliação atualizada em modo demonstração.", payload, id));
    [HttpDelete("reviews/{id}")] public Task<IActionResult> DeleteReview(string id) => ProxyDelete($"/api/reviews/{Uri.EscapeDataString(id)}", DemoMutation("Avaliação removida em modo demonstração.", id));
