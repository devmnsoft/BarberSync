(() => {
  ...
  let results = quickActions, active = 0, timer, request;
  ...
  async function search(query) {
    ...
    if (host) host.innerHTML = '<div class="command-state command-loading"><i></i><span>Buscando somente na unidade ativa…</span></div>';
    try {
      const response = await fetch(`/AdminApi/search?q=${encodeURIComponent(query)}`, { headers: { Accept: 'application/json' }, signal: request.signal });
      if (!response.ok) { const problem = await response.json().catch(() => ({})); throw new Error(problem.detail || problem.message || `Falha HTTP ${response.status}`); }
      ...
      results = [];
      if (host) host.innerHTML = `<div class="command-state command-error"><strong>Não foi possível buscar agora.</strong><span>${escape(error.message)}</span><button type="button" class="btn btn-light" data-command-retry>Tentar novamente</button></div>`;
  ...
  document.addEventListener('click', event => { if (event.target.closest('[data-command-close]')) close(); if (event.target.closest('[data-command-retry]')) search(elements().input.value.trim()); const item = event.target.closest('[data-command-index]'); if (item) execute(Number(item.dataset.commandIndex)); });
