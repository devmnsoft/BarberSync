window.renderAdminCrudPage = async function(module){
  const endpointMap={Clients:'/api/clients',Professionals:'/api/professionals',Services:'/api/services',Appointments:'/api/appointments',ServiceOrders:'/api/service-orders',Stock:'/api/stock/critical',Campaigns:'/api/campaigns',Coupons:'/api/coupons',Loyalty:'/api/loyalty/summary',Reviews:'/api/reviews',Copilot:'/api/copilot/suggestions',Products:'/api/products'};
  const demo=[{name:`Demo ${module} 1`,status:'Ativo'},{name:`Demo ${module} 2`,status:'Ativo'},{name:`Demo ${module} 3`,status:'Pendente'}];
  const {data,fallback}=await adminApiClient.get(endpointMap[module]||'/api/dashboard/summary',demo);
