(() => {
  const fallback = { kpis:[
    ...
    copilotSuggestions:[{title:'Retenção',description:'Acionar clientes inativos há 45 dias.'},{title:'Escala',description:'Reforçar equipe entre 18h e 20h.'},{title:'Estoque',description:'Repor insumos críticos antes do fim de semana.'},{title:'Upsell',description:'Sugerir combo Corte + Barba.'},{title:'Reputação',description:'Pedir avaliações após pagamento PIX.'},{title:'Fidelidade',description:'Dobrar cashback para VIPs sem visita.'}]
  };
  const icons=['💰','📈','📅','👥','✂️','🧾','🎯','📦','⭐','📣','🖥️','💈'];
  ...
  const render=(payload)=>{
    ...
    const st=document.querySelector('[data-stock-critical]'); if(st) st.innerHTML=list(data.stockCritical||fallback.stockCritical,s=>`<li><strong>${s.name}</strong><span>${s.current ?? s.currentStock ?? s.stock}/${s.minimum ?? s.minStock}</span></li>`);
