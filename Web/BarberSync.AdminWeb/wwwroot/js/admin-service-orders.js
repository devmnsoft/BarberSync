(() => {
  ...
  q('#posItems').addEventListener('click', async event => { const target = event.target.closest('[data-remove-item]'); if (!target) return; const accepted = await window.BarberSyncConfirm?.ask?.({ title: 'Remover item?', message: 'O total da comanda será recalculado e a justificativa ficará na auditoria.', confirmText: 'Remover' }); if (accepted === false) return; const reason = window.prompt('Informe o motivo da remoção:')?.trim(); if (!reason) return toast('validation', 'O motivo da remoção é obrigatório.'); try { await api(`service-orders/${state.order.id}/items/${target.dataset.removeItem}`, { method: 'DELETE', body: JSON.stringify({ reason }) }); await selectOrder(state.order.id); toast('updated', 'Item removido.'); } catch (error) { handle(error); } });
