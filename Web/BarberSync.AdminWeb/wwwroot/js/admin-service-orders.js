(() => {
  ...
  function renderOrder() {
    ...
    q('#posItems').innerHTML = order?.items?.length ? order.items.map(item => `<article class="pos-cart-item"><span class="pos-item-type">${item.type === 'Product' ? '▣' : '✂'}</span><div><strong>${esc(item.description)}</strong><small>${esc(nameOf(state.professionals, item.professionalId, item.professionalId ? 'Profissional' : 'Sem profissional'))}</small><span>${item.quantity} × ${money(item.unitPrice)}${Number(item.discount) ? ` · desconto ${money(item.discount)}` : ''}</span></div><b>${money(item.total)}</b><button type="button" data-remove-item="${esc(item.id)}" ${canEdit ? '' : 'disabled'} aria-label="Remover ${esc(item.description)}">×</button></article>`).join('') : '<div class="pos-empty"><strong>Comanda sem itens</strong><span>Escolha um serviço ou produto no catálogo.</span></div>';
