window.KioskFlow={
  ...
  setState(patch){
    ...
    sessionStorage.setItem('kiosk-flow',JSON.stringify(next));
  ...
  async post(url, body, fallback){ try{ const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); if(!r.ok) throw new Error(`HTTP ${r.status}`); return await r.json(); } catch (error) { console.warn('[BarberSync KioskFlow] fallback demo', error?.message || error); return fallback; } },
