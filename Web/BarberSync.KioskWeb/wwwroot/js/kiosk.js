(() => {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('kioskPay')?.addEventListener('click',async()=>{ const commandNumber=`K-${Date.now().toString().slice(-5)}`; const next=KioskFlow.setState({paymentMethod:KioskFlow.state.paymentMethod||'PIX', commandNumber, paidAt:new Date().toISOString(), status:'Atendimento registrado com sucesso'}); await KioskFlow.post('/KioskApi/payment/mock',next,{success:true}); sessionStorage.setItem('kiosk-summary', JSON.stringify(next)); location.href='/Kiosk/Success';});
    ...
    document.getElementById('kioskReviewForm')?.addEventListener('submit',async(e)=>{e.preventDefault(); const review=Object.fromEntries(new FormData(e.target).entries()); const summary={...KioskFlow.state, review}; sessionStorage.setItem('kiosk-summary', JSON.stringify(summary)); await KioskFlow.post('/KioskApi/review',review,{success:true}); location.href='/Kiosk/Summary';});
    const finalSummary=document.getElementById('kioskFinalSummary'); if(finalSummary){ const s=JSON.parse(sessionStorage.getItem('kiosk-summary')||sessionStorage.getItem('kiosk-flow')||'{}'); finalSummary.innerHTML=`<p><strong>Serviço:</strong> ${s.serviceName||'Corte Masculino'}</p><p><strong>Cliente:</strong> ${s.client?.name||'Cliente demo'}</p><p><strong>Profissional:</strong> ${s.professionalName||'Primeiro disponível'}</p><p><strong>Pagamento:</strong> ${s.paymentMethod||'PIX'}</p><p><strong>Número da comanda:</strong> ${s.commandNumber||'K-204'}</p><p><strong>Avaliação:</strong> ${s.review?.rating||'Pendente'} ⭐</p>`; }
  });
})();
